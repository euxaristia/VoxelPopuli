...
species! {
    ...
    Cow, "Cow", "cow", Grazer, Walk, Passive, Grassland, 1.3, 0.9, 10.0, 2.15, 0, [87,64,49], [231,226,209];
    ...
    Axolotl, "Axolotl", "axolotl", Canine, Amphibious, Passive, Special, 0.42, 0.75, 14.0, 1.5, 2, [238,175,190], [183,60,110];
    Bat, "Bat", "bat", Bat, Fly, Passive, Cave, 0.9, 0.5, 6.0, 1.6, 0, [87,61,48], [155,116,84];
    Bee, "Bee", "bee", Bee, Fly, Neutral, Grassland, 0.5, 0.55, 10.0, 3.0, 2, [232,181,58], [79,56,38];
    Bogged, "Bogged", "bogged", Person, Walk, Hostile, Special, 1.9, 0.6, 16.0, 1.5, 4, [140,146,102], [76,116,66];
    ...
    Cat, "Cat", "cat", Cat, Walk, Passive, Village, 0.56, 0.48, 10.0, 2.4, 0, [166,128,89], [227,204,161];
    CaveSpider, "Cave Spider", "cave_spider", Spider, Walk, Hostile, Special, 0.5, 0.7, 12.0, 1.9, 2, [39,70,73], [159,36,44];
    Chicken, "Chicken", "chicken", Bird, Walk, Passive, Grassland, 0.8, 0.6, 4.0, 1.6, 0, [235,230,211], [210,62,49];
    Cod, "Cod", "cod", Fish, Swim, Passive, Ocean, 0.3, 0.5, 3.0, 1.5, 0, [149,123,78], [210,194,148];
    CopperGolem, "Copper Golem", "copper_golem", Golem, Walk, Passive, Special, 0.98, 0.6, 12.0, 1.3, 0, [181,105,65], [89,160,126];
    ...
    Donkey, "Donkey", "donkey", Horse, Walk, Passive, Grassland, 1.6, 1.4, 20.0, 2.0, 0, [127,118,100], [203,191,163];
    Drowned, "Drowned", "drowned", Person, Amphibious, Hostile, Ocean, 1.9, 0.6, 20.0, 1.4, 3, [59,123,124], [115,158,121];
    ...
    Evoker, "Evoker", "evocation_illager", Villager, Walk, Hostile, Special, 1.9, 0.6, 24.0, 1.6, 6, [49,48,54], [133,154,143];
    Fox, "Fox", "fox", Canine, Walk, Passive, Taiga, 0.7, 0.6, 10.0, 2.4, 0, [207,120,49], [235,219,181];
    Frog, "Frog", "frog", Frog, Amphibious, Passive, Special, 0.55, 0.5, 10.0, 1.0, 0, [168,119,72], [211,185,115];
    ...
    Horse, "Horse", "horse", Horse, Walk, Passive, Grassland, 1.6, 1.4, 24.0, 2.7, 0, [140,88,52], [223,205,172];
    Husk, "Husk", "husk", Person, Walk, Hostile, Desert, 1.9, 0.6, 20.0, 1.5, 3, [135,112,73], [164,144,98];
    Llama, "Llama", "llama", Horse, Walk, Neutral, Mountains, 1.87, 0.9, 24.0, 1.8, 2, [207,187,142], [116,86,55];
    Mooshroom, "Mooshroom", "mooshroom", Grazer, Walk, Passive, Special, 1.3, 0.9, 10.0, 2.15, 0, [171,47,41], [232,220,196];
    Mule, "Mule", "mule", Horse, Walk, Passive, Special, 1.6, 1.4, 24.0, 2.2, 0, [99,76,53], [189,166,125];
    Nautilus, "Nautilus", "nautilus", Nautilus, Swim, Neutral, Ocean, 0.95, 0.875, 20.0, 2.0, 2, [219,172,113], [157,79,50];
    ...
    Parrot, "Parrot", "parrot", Bird, Fly, Passive, Special, 1.0, 0.5, 6.0, 1.8, 0, [202,60,52], [63,138,167];
    Parched, "Parched", "parched", Person, Walk, Hostile, Desert, 1.9, 0.6, 16.0, 1.5, 4, [183,158,105], [104,94,61];
    Phantom, "Phantom", "phantom", Bat, Fly, Hostile, Special, 0.5, 0.9, 20.0, 3.0, 4, [66,91,125], [162,199,105];
    Pillager, "Pillager", "pillager", Villager, Walk, Hostile, Special, 1.9, 0.6, 24.0, 1.8, 4, [99,77,74], [139,155,147];
    PolarBear, "Polar Bear", "polar_bear", Bear, Walk, Neutral, Snow, 1.4, 1.4, 30.0, 1.5, 6, [229,228,207], [91,83,71];
    Pufferfish, "Pufferfish", "pufferfish", Guardian, Swim, Neutral, Ocean, 0.5, 0.5, 3.0, 1.0, 2, [210,184,65], [139,103,47];
    Rabbit, "Rabbit", "rabbit", Rabbit, Hop, Passive, Grassland, 0.6, 0.49, 3.0, 1.7, 0, [166,139,101], [223,206,175];
    Ravager, "Ravager", "ravager", Bear, Walk, Hostile, Special, 2.2, 1.95, 100.0, 1.6, 8, [107,110,104], [185,180,155];
    Salmon, "Salmon", "salmon", Fish, Swim, Passive, River, 0.5, 0.5, 3.0, 1.8, 0, [153,77,67], [82,116,86];
    Silverfish, "Silverfish", "silverfish", Armadillo, Walk, Hostile, Special, 0.3, 0.4, 8.0, 1.5, 1, [126,134,136], [74,83,89];
    SkeletonHorse, "Skeleton Horse", "skeleton_horse", Horse, Walk, Passive, Special, 1.6, 1.4, 15.0, 2.7, 0, [201,199,177], [87,87,78];
    Slime, "Slime", "slime", Slime, Hop, Hostile, Cave, 1.04, 1.04, 16.0, 1.3, 3, [103,171,72], [57,100,48];
    Sniffer, "Sniffer", "sniffer", Sniffer, Walk, Passive, Special, 1.75, 1.9, 14.0, 1.1, 0, [89,132,67], [150,65,48];
    SnowGolem, "Snow Golem", "snow_golem", Golem, Walk, Passive, Special, 1.8, 0.4, 4.0, 1.2, 0, [226,234,227], [210,123,42];
    ...
    Stray, "Stray", "stray", Person, Walk, Hostile, Snow, 1.9, 0.6, 20.0, 1.6, 4, [169,190,192], [77,115,124];
    SulfurCube, "Sulfur Cube", "sulfur_cube", Slime, Hop, Passive, Special, 0.98, 0.98, 16.0, 1.3, 0, [199,210,119], [93,137,114];
    ...
    TropicalFish, "Tropical Fish", "tropicalfish", Fish, Swim, Passive, Ocean, 0.52, 0.52, 3.0, 1.5, 0, [230,142,63], [235,220,183];
    ...
    Vindicator, "Vindicator", "vindicator", Villager, Walk, Hostile, Special, 1.9, 0.6, 24.0, 2.0, 7, [66,83,89], [139,154,146];
    WanderingTrader, "Wandering Trader", "wandering_trader", Villager, Walk, Passive, Special, 1.9, 0.6, 20.0, 1.7, 0, [61,109,165], [207,165,83];
    Warden, "Warden", "warden", Golem, Walk, Hostile, Special, 2.9, 0.9, 500.0, 1.4, 15, [34,74,73], [95,194,175];
    Witch, "Witch", "witch", Villager, Walk, Hostile, Special, 1.9, 0.6, 26.0, 1.6, 4, [99,64,120], [143,154,108];
    Wither, "Wither", "wither", Wither, Fly, Hostile, Special, 3.0, 1.0, 300.0, 1.4, 10, [57,61,64], [145,151,151];
    Wolf, "Wolf", "wolf", Canine, Walk, Neutral, Taiga, 0.8, 0.6, 20.0, 2.4, 4, [175,173,157], [230,222,199];
    ZombieHorse, "Zombie Horse", "zombie_horse", Horse, Walk, Passive, Night, 1.6, 1.4, 15.0, 2.5, 0, [102,135,79], [61,79,52];
    ZombieNautilus, "Zombie Nautilus", "zombie_nautilus", Nautilus, Swim, Neutral, Ocean, 0.95, 0.875, 20.0, 2.0, 2, [117,150,108], [67,94,74];
    ZombieVillager, "Zombie Villager", "zombie_villager_v2", Villager, Walk, Hostile, Night, 1.9, 0.6, 20.0, 1.5, 3, [130,100,67], [110,148,77];
    ZombifiedPiglin, "Zombified Piglin", "zombie_pigman", Person, Walk, Neutral, Special, 1.9, 0.6, 20.0, 1.8, 5, [159,115,97], [106,139,76];
}
...
mod tests {
    use super::*;
    #[test]
    fn adult_collision_dimensions_match_bedrock_components() {
        for line in include_str!("../tests/fixtures/bedrock-mob-dimensions.txt")
            .lines()
            .filter(|l| !l.starts_with('#'))
        {
            let fields: Vec<_> = line.split_whitespace().collect();
            let s = MobKind::from_id(fields[0]).unwrap().species();
            let expected = (
                fields[1].parse::<f32>().unwrap(),
                fields[2].parse::<f32>().unwrap(),
            );
            let scale = fields[4].parse::<f32>().unwrap();
            assert!(
                (s.height - expected.0 * scale).abs() < 1e-6
                    && (s.width - expected.1 * scale).abs() < 1e-6,
                "{} ({}) expected {:?} scaled by {} but got {:?}",
                fields[0],
                fields[3],
                expected,
                scale,
